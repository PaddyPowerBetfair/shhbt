...
class TestIntegrations(TestCase):
    ...
    def test_skips_and_succeeds_when_deleted_file(self, req_mock, status_mock):
        ...
        assert self.test_client.post("/", headers={"X-Gitlab-Event": "test-event"}, json=api_json_res.EVENT_FOR_UNSAFE).status_code == 200
    ...
    def test_returns_400_if_not_merge_request(self):
        assert self.test_client.post("/", headers={"X-Gitlab-Event": "test-event"}, json={"event_type": "comment"}).status_code == 400
